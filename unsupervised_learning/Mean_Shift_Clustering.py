...
if file:
    ...
    bandwidth = st.sidebar.slider("Choose value of bandwidth", min_value=1.0, max_value=10.0, key='k', value=2.0,
                                     step=0.1)
    model = MeanShift(bandwidth=bandwidth)
    model.fit(x)

    always_virginca = [[7.7, 2.6, 6.9, 2.3]]
    always_setosa = [[5.7, 4.4, 1.5, 0.4]]
    always_versicolor = [[5.1, 2.5, 3, 1.1]]

    label_versicolor = model.predict(always_virginca).item()
    label_setosa = model.predict(always_setosa).item()
    label_virginica = model.predict(always_versicolor).item()
